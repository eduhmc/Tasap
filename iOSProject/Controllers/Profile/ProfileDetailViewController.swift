...
class ProfileDetailViewController: UIViewController {
    ...
    fileprivate func baseQuery() -> Query {
        return user!.document!.collection("reviews")
    }
    ...
    func setup(){
        ...
        firstNameLabel.text = "\(user!.first) \(user!.last)"
        emailLabel.text = user!.email
        priceLabel.text = "$\(user!.price)"
        ratingLabel.text = String(format: "%.1f", user!.ratingAverage)
    ...
    @IBAction func coursesUpdateButtonTapped(_ sender: Any) {
        self.performSegue(withIdentifier: "updateCoursesSegue", sender: user)
        
    }
    
    @IBAction func eventsUpdateButtonTapped(_ sender: Any) {
        self.performSegue(withIdentifier: "calendarSegue", sender: user)
        ...
        let buttonTwo = DefaultButton(title: "UPDATE", height: 60) {
            ...
            self.user!.document!.updateData([
                ...
                if let err = err {
                    ...
                    self.user!.price = price
                    AuthenticationManager.shared.currentUser = self.user
    ...
    func showDescriptionDialog(animated: Bool = true) {
        ...
        editVC.textComment = user!.description
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "updateCoursesSegue" {
            let destinationVC = segue.destination as! UpdateCareerViewController
            destinationVC.user = user
        }else if segue.identifier == "calendarSegue" {
            let destinationVC = segue.destination as! CalendarViewController
            destinationVC.user = user
            destinationVC.isOnlyView = false
        }
    }
...
extension ProfileDetailViewController: CoachMarksControllerDataSource {
    ...
    @objc func gotoCalendar(){
        if let user = self.user {
            performSegue(withIdentifier: "calendarSegue", sender: user)
        }
    }
